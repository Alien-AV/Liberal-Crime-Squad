 ...
 * Copyright 2009, 2013, 2014 Jonathan Stickles  <jonathansfox@users.sourceforge.net>
 * Copyright 2009 Ari Rahikkala  (arirahikkala)
 * Copyright 2010 mdownie
...
int
findlocation(int type, int city = -1)
{
  if (!multipleCityMode)
    city = -1;
  for (int i = 0; i < len(location); i++)
    if (location[i]->type == type && (location[i]->city == city || city == -1))
      return i;
  return -1;
}
 ...
 *  1: first part of the name is short unless the place itself is a city in which case it's long,
 * and if there's a city at the end it's short
 *  2: entire name is short, no matter what */
string
Location::getname(signed char shortname_, bool include_city)
{
  string str;
  if (!multipleCityMode)
    include_city = false;
  ...
  if (include_city && type != city)
  {
    ...
    if (str == "Downtown")
      return str + " " + cityname;
    if (str == "University District" || str == "U-District" || str == "Industrial District"
        || str == "I-District"
        || str == "Shopping"
        || str == "Outskirts"
        || str == "Seaport Area"
        || str == "Seaport"
        || str == "Outskirts & Orange County")
      return cityname + " " + str;
    ...
    if (str == "Manhattan" || str == "Manhattan Island" || str == "Brooklyn & Queens"
        || str == "Long Island"
        || str == "The Bronx")
...
bool
Location::can_be_fortified()
{
  if (!upgradable)
    return false;
  switch (type)
  {
    default: return !fortified();
    case SITE_OUTDOOR_BUNKER:
    case SITE_BUSINESS_BARANDGRILL: return false;
  }
}
...
Location::fortified()
{
  switch (type)
  {
    default: return compound_walls & COMPOUND_BASIC;
    case SITE_OUTDOOR_BUNKER: return true;
  }
}


bool
Location::can_be_trapped()
{
  if (!upgradable)
    return false;
  return !trapped();
...
bool
Location::can_install_tanktraps()
{
  if (!upgradable)
    return false;
  switch (type)
  {
    default: return !tank_traps();
    case SITE_BUSINESS_BARANDGRILL:
    case SITE_OUTDOOR_BUNKER:
    case SITE_RESIDENTIAL_BOMBSHELTER: return false;
  }
}
...
Location::tank_traps()
{
  switch (type)
  {
    default: return compound_walls & COMPOUND_TANKTRAPS;
    case SITE_OUTDOOR_BUNKER:
    case SITE_RESIDENTIAL_BOMBSHELTER: return true;
  }
}


bool
Location::can_have_businessfront()
{
  if (!upgradable)
    return false;
  switch (type)
  {
    default: return !has_business_front();
    case SITE_BUSINESS_BARANDGRILL:
    case SITE_OUTDOOR_BUNKER:
    case SITE_RESIDENTIAL_BOMBSHELTER: return false;
  }
...
bool
Location::has_business_front()
{
  return front_business != -1 || type == SITE_BUSINESS_BARANDGRILL;
}


bool
Location::bomb_resistant()
{
...
bool
Location::part_of_justice_system()
{
  return type == SITE_GOVERNMENT_POLICESTATION || type == SITE_GOVERNMENT_COURTHOUSE
...
Location::update_heat_protection()
{
  ...
  {
  } // None if it isn't and you have no flag

  if (heat_protection < 0)
    heat_protection = 0;

  heat_protection *= 5;
  if (heat_protection > 95)
    heat_protection = 95;
}


void
Location::rename(const char* name_, const char* shortname_)
{
...
{ // NOTE: make sure to keep code here matching code in updateworld_laws() in monthly.cpp for when
  // names are changed
  loc.init();
  ...
  switch (loc.type)
  {
    ...
    case SITE_GOVERNMENT_INTELLIGENCEHQ:
      if (law[LAW_PRIVACY] == Alignment::ARCH_CONSERVATIVE
    ...
    case SITE_GOVERNMENT_ARMYBASE:
      ...
      break;
    case SITE_GOVERNMENT_WHITE_HOUSE: loc.rename("White House", "White House"); break;
    case SITE_CORPORATE_HEADQUARTERS: loc.rename("Corporate HQ", "Corp. HQ"); break;
    case SITE_BUSINESS_BANK: loc.rename("American Bank Corp", "Bank"); break;
    ...
    case SITE_CORPORATE_HOUSE:
      if (law[LAW_CORPORATE] == Alignment::ARCH_CONSERVATIVE
          && law[LAW_TAX] == Alignment::ARCH_CONSERVATIVE)
        loc.rename("CEO Castle", "CEO Castle");
      else
        loc.rename("CEO Residence", "CEO House");
      break;
    case SITE_RESIDENTIAL_SHELTER: loc.rename("Homeless Shelter", "Shelter"); break;
    case SITE_INDUSTRY_WAREHOUSE:
      do
      {
    ...
    case SITE_INDUSTRY_POLLUTER:
      switch (LCSrandom(5))
      {
        case 0: loc.rename("Aluminum Factory", "Alum Fact"); break;
        case 1: loc.rename("Plastic Factory", "Plast Fact"); break;
        case 2: loc.rename("Oil Refinery", "Refinery"); break;
        case 3: loc.rename("Auto Plant", "Auto Plant"); break;
        case 4: loc.rename("Chemical Factory", "Chem Fact"); break;
      }
      break;
    case SITE_MEDIA_CABLENEWS: loc.rename("Cable News Station", "News Station"); break;
    case SITE_MEDIA_AMRADIO: loc.rename("AM Radio Station", "Radio Station"); break;
    case SITE_RESIDENTIAL_APARTMENT_UPSCALE:
      do
      {
        lastname(loc.name, true);
    ...
    case SITE_RESIDENTIAL_TENEMENT:
      ...
      break;
    case SITE_HOSPITAL_UNIVERSITY: loc.rename("The University Hospital", "U Hospital"); break;
    case SITE_HOSPITAL_CLINIC: loc.rename("The Free Clinic", "Clinic"); break;
    ...
    case SITE_BUSINESS_DEPTSTORE:
      ...
      break;
    case SITE_BUSINESS_HALLOWEEN: loc.rename("The Oubliette", "Oubliette"); break;
    ...
    case SITE_OUTDOOR_PUBLICPARK:
      ...
      break;
    case SITE_RESIDENTIAL_BOMBSHELTER: loc.rename("Fallout Shelter", "Bomb Shelter"); break;
    case SITE_BUSINESS_BARANDGRILL: loc.rename("Desert Eagle Bar & Grill", "Bar & Grill"); break;
    case SITE_OUTDOOR_BUNKER: loc.rename("Robert E. Lee Bunker", "Bunker"); break;
    case SITE_BUSINESS_ARMSDEALER: loc.rename("Black Market", "Black Market"); break;
  }
}


/* transfer all loot from some source (such as a squad or another location) to a location, and deal
 * with money properly */
void
Location::getloot(vector<Item*>& loot)
{
